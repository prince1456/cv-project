...
import Experience5 from "./experience5/Experience5";
const ExperienceStep = (props) => {
  ...
  const getEditableInputs = (exp, idx) => {
    ...
    if (selectedTemplate.id === 5) {
      divClass = classes.ExperienceInput5;
      primaryTitle = "InlinePrimaryTitle5";
      secondaryTitle = "InlineSecondaryTitle5";
      tertiaryTitle = "InlineTertiaryTitle5";
  ...
  if (data.experiences) {
    experiences = data.experiences.map((exp, idx) => {
      return <Aux key={idx}>{getEditableInputs(exp, idx)}</Aux>;
    });
  }
  ...
  if (selectedTemplate.id === 4) {
    content = (
      <div className={classes.Wrapper}>
        ...
        <div className={classes.TitleWrapper}>
          <label className={classes.StepTitle}>Experience</label>
        </div>
        <div className={classes.TextBox}>
          <div className={classes.TextBoxInner}>{experiences}</div>
          <button
    ...
    const bgUrl = selectedTemplate.sceneBackgrounds.find((s) =>
      props.id.includes(s.id)
    ).url;
    content = (
      <Experience5
      ...
      <div className={classes.Wrapper}>
        ...
        <div className={classes.TextBox9}>
          <div className={classes.TextBoxInner9}>
            <div className={classes.ExperienceBox9}>{experiences}</div>
          </div>
          <button
            disabled={data.experiences && data.experiences.length === 4}
            style={{
